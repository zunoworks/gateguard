# AIに「保険」をかける — 6バージョン潜んだバグが教えてくれた、ガードレールの正しい作り方

## 2026年、事故は「攻撃」ではなく「日常作業」から起きている

今年報告されたAIコーディング事故には共通の形がある。

- ルーチンな作業指示（「復元して」「掃除して」）
- もっともらしい1ステップ（`rm -rf`、復元スクリプトの実行）
- **誰も中身を見る前に、取り返しのつかないことが起きる**

本番データベースがバックアップごと数秒で消えた事例に、攻撃者はいなかった。いたのは、確認せずに動いたAIと、それを止める仕組みの不在だけだ。
